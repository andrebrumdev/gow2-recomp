...
if [ "$SIGN_ONLY" = 1 ]; then
    for f in "$B/libgow2_game.a" "$B/rt/libps3recomp_runtime.a" "$B/sdl-root/sdl/lib/libSDL2.a" "$B/sdl-root/sdl/lib/libSDL2main.a" \
             "$B/ffmpeg-ios/lib/libavcodec.a" "$B/ffmpeg-ios/lib/libavutil.a" "$B/ffmpeg-ios/share/licenses/FFmpeg-COPYING.LGPLv2.1.txt"; do
    ...
    # cellVdec's software MPEG-2 (FFmpeg, LGPL-2.1, mpeg2video only); stamped: rebuilt only when the pin changes
    "$PS3/tools/ios/build_ffmpeg.sh" ios "$B/ffmpeg-ios" | tail -1
    "$PS3/tools/ios/build_runtime_ios.sh" "$B/rt" "$B/sdl-root/sdl" "$B/ffmpeg-ios"
fi
rm -rf "$IOS_HERE/Generated/licenses"           # FFmpeg's license and notes travel inside the app
...
FW="Metal MetalFX MetalPerformanceShaders QuartzCore CoreGraphics Foundation UIKit AVFoundation CoreMedia CoreVideo VideoToolbox AudioToolbox CoreAudio GameController CoreHaptics CoreText CoreMotion OpenGLES ImageIO MobileCoreServices CoreBluetooth"
{
    ...
    printf '\n'
    printf 'GOW2_IOS_LDFLAGS = -force_load %s %s %s %s %s %s' "$B/libgow2_game.a" "$B/rt/libps3recomp_runtime.a" \
