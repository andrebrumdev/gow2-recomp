...
echo "=== run SÓ PS3_SYSUTIL_BOOT=1 (limpo, sem force) (12s) $(date +%H:%M:%S) ==="
PS3_VFS_ROOT="/c/Users/softlive/Documents/self-projects/gow2_work/extracted" PS3_SYSUTIL_BOOT=1 PS3_TRACE_SPURS=all PS3_TRACE_SC=1 PS3_TRACE_T=1 timeout -k 3 12 ./boot_hle.exe ../EBOOT.ELF > cb.out 2> cb.err
echo "=== spin saiu? (141 max count — baixo=saiu) ==="
grep -aoE "num=141 #[0-9]+" cb.err | grep -oE "[0-9]+$" | sort -n | tail -1
echo "=== obj construído? T-traces (T1-T4 de func_0024A7CC) ==="
grep -aoE "\[T[0-9]+\]" cb.err | sort | uniq -c | head
echo "=== HLE total (era 380) + avançou? ==="
echo "HLE: $(grep -ac '^\[TRACE\]' cb.err)"
grep -aE "^\[TRACE\]" cb.err | grep -oaE "(_)?(cell|sce|sys)[A-Za-z_]+" | tail -30 | awk '!seen[$0]++' | tail -16
echo "=== crash/FATAL/OOB? ==="
grep -aiE "unresolved indirect|OOB access|FATAL|Segmentation" cb.err | head -4
echo "=== stdout tail ==="; tail -5 cb.out
echo FIM
