    ...
    for f in sorted(ROOT.glob("ppu_recomp_00*.cpp")):
        ...
        if i>=0:
            ...
            if 0<=j<e and "E147-SND" not in s[i:e]:
                k=s.find("\n",j)
                s=s[:k]+"\n        /* E147-SND */ "+GATE+r''' if(_on){ static int _k=0; if(_k++<12){ uint32_t _s=(uint32_t)ctx->gpr[3]; int _ok=(_s>=0x10000u&&_s<0x4F000000u); uint32_t _i=_ok?vm_read32(_s+0x1ecu):0u; uint32_t _b=_s+_i*0x308u;
            fprintf(stderr,"[SNDTIME] stream=0x%08X idx=%u +0x519=%u estado=%u samples(+0x154)=%u rate=0x%08X +0x348=0x%08X\n",_s,_i,_ok?vm_read8(_b+0x519u):0u,_ok?vm_read32(_b+0x358u):0u,_ok?vm_read32(_s+0x154u):0u,_ok?vm_read32(_b+0x364u):0u,_ok?vm_read32(_b+0x348u):0u); } } }'''+s[k:]; n+=1
