...
Gate (default OFF):
  PS3_TYPE15_DISC=1   — preferred; does NOT enable the spammy PS3_TRACE_CC9D0
                        [CC9D0] iter logger.
  PS3_TRACE_CC9D0=1   — also enables these DISC lines (same env_on helper as
                        the offline test); existing [CC9D0] spam still separate.
...
# Same helper as test_trace_cc9d0_env.py (kept in sync for offline assert).
...
def _safe_vm_helpers_block() -> str:
    """C++ snippet: env gate + safe read + shared budget check."""
    return r'''
        /* M1 TYPE15/CC9D0 disc (PS3_TYPE15_DISC=1 or PS3_TRACE_CC9D0=1; ≤20 lines). */
        { static int _disc_on = -1;
          if (_disc_on < 0) {
            ...
            int on1 = 0, on2 = 0;
            if (e1) { const char* p=e1; while(*p==' '||*p=='\t') p++;
              if (*p && *p!='0') {
                /* reject false/off/no (case-insensitive first letter) */
                char c = (*p>='A'&&*p<='Z') ? (char)(*p+32) : *p;
                if (!(c=='f'||c=='n'|| (c=='o' && p[1] && (p[1]=='f'||p[1]=='F')))) on1=1;
              }
            }
            if (e2) { const char* p=e2; while(*p==' '||*p=='\t') p++;
              if (*p && *p!='0') {
                char c = (*p>='A'&&*p<='Z') ? (char)(*p+32) : *p;
                if (!(c=='f'||c=='n'|| (c=='o' && p[1] && (p[1]=='f'||p[1]=='F')))) on2=1;
              }
            }
            _disc_on = (on1 || on2) ? 1 : 0;
          }
'''
...
def patch(t: str) -> str:
    ...
    cb56c_full_insert = '''            { static int _n=0; if(_n++<8)
                fprintf(stderr,"[POSTINTRO] CB56C after 2A4FE4\\n"); }
            ...
            { static int _disc_on = -1;
              if (_disc_on < 0) {
                ...
                const char* e1 = getenv("PS3_TYPE15_DISC");
                const char* e2 = getenv("PS3_TRACE_CC9D0");
                _disc_on = ((e1 && *e1 && *e1!='0') || (e2 && *e2 && *e2!='0')) ? 1 : 0;
              }
    ...
    cc9d0_insert = '''        { static int on=-1; if(on<0){extern char* getenv(const char*);
            const char* e=getenv("PS3_TRACE_CC9D0");
            on=(e&&*e&&*e!='0')?1:0;}
          if(on){ static long n=0; static uint32_t last_this=0xFFFFFFFFu;
            uint32_t th=(uint32_t)ctx->gpr[31];
