...
def main():
    ...
    for f in sorted(ROOT.glob("ppu_recomp_00*.cpp")):
        ...
        if h in s and "E150-SUBMIT" not in s:
            s=s.replace(h,h+"\n        /* E150-SUBMIT */ "+GATE+r''' if(_on){ static int _k=0; if(_k++<60){ uint32_t _op=(uint32_t)ctx->gpr[4]; int _ok=(_op>=0x10000u&&_op<0x4F000000u); uint32_t _fh=_ok?vm_read32(_op+0x98u):0u; '''+PATH+r''' fprintf(stderr,"[FIOSOP] submit op=0x%08X tipo=0x%X fh=0x%08X path='%s' lr=0x%08X\n",_op,_ok?vm_read32(_op+0x40u):0u,_fh,_p,(uint32_t)ctx->lr); } } }''',1); n+=1
