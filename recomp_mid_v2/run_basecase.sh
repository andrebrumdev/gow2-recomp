...
LOG="$RUN_DIR/basecase_trace.log"
TIMEOUT_S="${1:-150}"
cd "$RUN_DIR" || exit 1
...
echo "=== BC-CTOR (raw seeds, first 40) ==="; grep -a '\[BC-CTOR\]' "$LOG" | head -40
echo "=== BC-CTOR count ==="; grep -ac '\[BC-CTOR\]' "$LOG"
echo "=== BC-CTOR REENT count ==="; grep -a '\[BC-CTOR\]' "$LOG" | grep -ac 'REENT'
echo "=== BC-GROW-OPEN (first 20) ==="; grep -a '\[BC-GROW-OPEN\]' "$LOG" | head -20
echo "=== BC-GROW-OPEN no-magic count ==="; grep -a '\[BC-GROW-OPEN\]' "$LOG" | grep -ac 'no-magic'
echo "=== BC-PEEK (first 20) ==="; grep -a '\[BC-PEEK\]' "$LOG" | head -20
echo "=== interleaving window (ctx around first raw) ==="; grep -an '\[BC-CTOR\]\|\[BC-GROW-OPEN\]\|\[BC-GROW-CLOSE\]\|\[BC-PEEK\]' "$LOG" | head -60
