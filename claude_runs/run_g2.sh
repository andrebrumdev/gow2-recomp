#!/bin/bash
# run_for.sh BINARY_NAME SHA256 LOG_NAME SECS [EXTRA_ENV_NAME=VALUE ...]
# One GoW2 boot with the user's launcher env (windowed) and PS3_TRACE_FPS=1 for SECS
# seconds, then stops it (TERM, then KILL by PID). For short event-bounded traces.
set -u
G="$HOME/Documents/PESSOAL/gow2-recomp"
BIN_NAME="$1"; WANT="$2"; LOG_NAME="$3"; SECS="$4"
shift 4 2>/dev/null || shift $#
case "$BIN_NAME" in g2[a-z0-9]*) ;; *) echo "bad binary name"; exit 2 ;; esac
...
PID=$!
echo "pid=$PID secs=$SECS extra=${EXTRA[*]+${EXTRA[*]}}"
n=0
while [ "$n" -lt "$SECS" ]; do
  sleep 1
  n=$((n + 1))
