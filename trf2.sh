...
echo "=== run: fence completion ==="
PS3_VFS_ROOT="/c/Users/softlive/Documents/self-projects/gow2_work/extracted" PS3_VM_LOW_MB=512 PS3_VM_STACK_MB=64 PS3_FORCE_GCMINIT=1 PS3_BUILD_SINGLETON=1 PS3_SEM_SPIN_BREAK=4096 PS3_RSX_FIFO=1 PS3_FIX_DISPLAYLIST=1 PS3_TRACE_SPURS=all timeout -k 3 16 ./boot_hle.exe ../EBOOT.ELF > f2.out 2> f2.err
echo "exit=$?"
echo "=== passou do spin? GetLabel + flips + HLE + último ==="; echo "  GetLabel=$(grep -ac cellGcmGetLabelAddress f2.err) flips=$(grep -ac _cellGcmSetFlipCommand f2.err) HLE=$(grep -ac '^\[TRACE\]' f2.err) último=$(grep -aE '^\[TRACE\]' f2.err | grep -oaE '(_)?(cell|sce|sys)[A-Za-z_]+' | tail -1)"
echo "=== TOP HLE (render loop? múltiplos flips?) ==="; grep -aoE "^\[TRACE\] (_)?(cell|sce|sys)[A-Za-z_]+" f2.err | sed 's/\[TRACE\] //' | sort | uniq -c | sort -rn | head -10
echo "=== HLE distintas NOVAS (avanço além do flip)? ==="; grep -aE "^\[TRACE\]" f2.err | grep -oaE "(_)?(cell|sce|sys)[A-Za-z_]+" | awk '!s[$0]++' | tail -20
echo FIM
